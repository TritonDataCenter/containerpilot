...
func TestValidConfigHealthChecks(t *testing.T) {
	...
	check0 := cfg.Checks[0]
	check1 := cfg.Checks[1]
	assertEqual(t, check0.Name, "check.checkA", "expected '%v' for check0.Name, but got '%v'")
	...
	assertEqual(t, check1.Name, "check.checkB", "expected '%v' for check1.Name, but got '%v'")
