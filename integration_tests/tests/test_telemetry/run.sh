...
for _ in $(seq 0 20); do
    sleep 1
    metrics=$(docker exec -i "$app" curl -s "${IP}:9090/metrics")
...
docker exec -i "$app" /check.sh "${IP}"

# Make we register and tear down telemetry service in Consul
docker exec -i "$consul" assert service containerpilot 1
docker-compose stop app
docker exec -i "$consul" curl -s --fail localhost:8500/v1/catalog/service/containerpilot | grep -v 'containerpilot'
