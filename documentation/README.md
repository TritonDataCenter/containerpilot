...
ContainerPilot registers the application with a service catalog (Consul and Etcd are both supported, and others can be added)  on container start and periodically sends TTL health checks to Consul; should the application fail then the service catalog will not receive the health check and once the TTL expires will no longer consider the application node healthy. Meanwhile, ContainerPilot runs background workers that poll the service catalog, checking for changes in dependent/upstream service, and calling an external executable on change.
...
Details of how to configure the telemetry endpoint and how the telemetry endpoint works can be found in the [telemetry README](https://www.joyent.com/containerpilot/docs/telemetry).
...
Details of how to configure tasks and how tasks work can be found in the [tasks README](https://www.joyent.com/containerpilot/docs/tasks).
